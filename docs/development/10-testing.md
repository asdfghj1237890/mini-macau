...
| `waste.test.ts` | 九型別的顏色/圖示名/排序鍵互不重複、IAM／DSPA 來源歸類、`pickWasteText` en→pt→zh fallback（DSPA 沒有英文）、`visibleWasteSites` 隱藏類型過濾（沒有隱藏時保留 array identity）、`countWasteByType`／`visibleWasteCount`、`wasteLegendRows`、`loadHiddenWasteTypes`/`saveHiddenWasteTypes` 的 round-trip 與壞資料容錯、焚化中心從 `power-facilities.json` 借記錄（id+type 雙重比對）、環保加Fun站與處理設施的可見性/計數、`formatWasteAmount`／`wasteMonthBars` 統計格式化、`buildWasteFeatures`／`buildWasteAreaFeatures`／`buildWasteBuildingFeatures` |
