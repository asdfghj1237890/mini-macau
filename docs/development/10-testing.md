...
| `religion.test.ts` | 五類別顏色、5 × 4 類別×kind 圖示名互不重複、顏色跟類別走不跟 kind、`pickReligionText` 以中文碑文為 fallback、`buildReligionFeatures` 的 `approximate` 旗標與 `rank`、`countReligionByKind`／`countReligionByCategory`、`filterReligionByCategory` 全開時保留 array identity、`loadReligionCategoriesOn`/`saveReligionCategoriesOn` round-trip 與壞資料容錯、`siteFaith` 依類別退回 |
| `oldMaps.test.ts` | `oldMapSourceSpec`（沒有 `tiles` 的圖版是單張 `image` source；有 `tiles` 的是 raster 瓦片 source，`bounds` 依 MapLibre 的西南東北順序、URL 補上 origin 且 `{z}/{x}/{y}` 樣板保持原樣、origin 結尾的斜線不會變成雙斜線）、`basemapBuildingsPaint`（沒有圖版時維持主題色方塊、有圖版時兩個主題都換成同一組紙色半透明值）、source／layer id 互轉（source id 本身不是 layer id）、`oldMapName`／`oldMapTitle` 語言 fallback（en → zh）、`oldMapYears` 只在刊行年不同時才顯示、`filterOldMaps` 沒有隱藏時保留 array identity、`loadHiddenOldMaps`/`saveHiddenOldMaps` 與 `loadOldMapsOpacity`/`saveOldMapsOpacity` round-trip、範圍夾住（0.2–1）與壞資料／storage 例外容錯 |
