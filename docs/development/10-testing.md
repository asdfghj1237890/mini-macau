...
| `oldMaps.test.ts` | source／layer id 互轉（source id 本身不是 layer id）、`oldMapName`／`oldMapTitle` 語言 fallback（en → zh）、`oldMapYears` 只在刊行年不同時才顯示、`filterOldMaps` 沒有隱藏時保留 array identity、`loadHiddenOldMaps`/`saveHiddenOldMaps` 與 `loadOldMapsOpacity`/`saveOldMapsOpacity` round-trip、範圍夾住（0.2–1）與壞資料／storage 例外容錯 |
