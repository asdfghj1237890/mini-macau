...
| `religion.test.ts` | 五類別顏色、5 × 4 類別×kind 圖示名互不重複、顏色跟類別走不跟 kind、`pickReligionText` 以中文碑文為 fallback、`buildReligionFeatures` 的 `approximate` 旗標與 `rank`、`countReligionByKind`／`countReligionByCategory`、`filterReligionByCategory` 全開時保留 array identity、`loadReligionCategoriesOn`/`saveReligionCategoriesOn` round-trip 與壞資料容錯 |
