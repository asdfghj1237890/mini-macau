...
播放層（`BusPlayback`）的節奏（2026-09-15）：worker 每批回覆後，播放頭以「時鐘 − 緩衝延遲」的位置在已驗證軌跡上內插。緩衝延遲原本取「回覆間隔 ×1.2」與「來回時間 ×1.5」的較大者；但 ≥10× 的對齊批次要等時鐘到達批次邊界才送出，下一批實際落地是「一個批次週期 ＋ 一次來回 ＋ 兩格排程」，兩者要相加才夠——30×／60× 每一兩秒就餓到凍結 1–3 格，再用 1.2× 追回，看起來就是巴士一頓一頓；步調又是硬夾在 0.8×–1.2×，緩衝目標抖一格就讓整隊車在 0.8× 與 1.2× 之間來回好幾秒。現在緩衝延遲 = 回覆間隔 ＋ 來回 ×1.25 ＋ 80 ms（上限 600 ms 不變），對齊批次另以「批次週期 ＋ 換算成整批的來回」為下限；步調改成 ±40 ms 死區的比例控制（超出死區每 800 ms 才到 0.8×／1.2× 極限），而且只有已驗證軌跡的餘量超過緩衝目標時才會跑快追趕，否則每批都會撞到軌跡末端停格。1× 切到 10×／30×／60× 時 `AsyncBusFrame` 先維持逐格請求，等播放頭落後量長到緩衝目標（這段播放頭以 0.8× 緩緩讓出）才切回對齊批次；原本一切換就對齊，緩衝空掉整整一個批次週期（切到 10× 凍 165 ms、60× 凍 231 ms）。量測用 `node scripts/inspect.mjs bus-playback 08:00 20 <倍速> 5` 的 `pace`（每格播放頭前進量相對時鐘：frozen／slow／nominal／fast／snap）、`lagMs`、`headroomMs`、`delayMs`；`BUS_SWITCH_AT=6000` 模擬第 6 秒從 1× 切上去並另列切換後 3 秒的分佈，`BUS_FRAME_MS=100` 模擬手機節奏，`BUS_PLAYHEAD_TRACE=1` 印逐格數列。修正前後（08:00 舊城區視窗、20 秒、單機無其他負載）：穩態 30× 的凍結由 5–6 次降到 1 次、60× 由 12–13 次降到 1 次，0.8×／1.2× 的畫格從三成以上降到 0（啟動追趕除外）；切換到 10×／30×／60× 的凍結由 165／132／231 ms 降到 0／33／0 ms，改為約 1.1 秒的 0.8–0.97× 緩降。代價是穩態落後量（面板的 delay 含這段播放延遲）由約 165–240 ms 增加到約 260–310 ms 實際時間，60× 時相當於多顯示約 9 秒延誤。
