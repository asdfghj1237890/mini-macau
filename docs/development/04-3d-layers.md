...
WATER 一層有九個 layer（連同動畫相位與脈衝分桶，供水約 90 個、供電約 90 個圖層）。2026-09-25 起它們不再隨每次樣式載入建立，而是第一次打開 WATER／POWER 時才由 `addWaterLayers`／`addPowerLayers` 建立（打開狀態下換底圖時由 `addCustomLayers` 結尾呼叫）；插入點是固定的相鄰圖層——供水在供電之下、兩者都在垃圾回收的面之下，標記在大賽車彎道之下——所以不論先開哪一個，最終順序都和以前一次建好時相同。建立後就保留，關閉時照舊以清空資料隱藏。點擊的 `queryRenderedFeatures` 清單會先濾掉尚不存在的圖層（缺一個就整個查詢失敗）。可見性跟著 App 傳入的 `waterVisible`（即 `waterOn`；管網類 layer 由 `applyNetworkVisibility` 切換，[`src/water.ts`](../../src/water.ts) 出 feature，[`MapView.tsx`](../../src/components/MapView.tsx) 管 layer）：
