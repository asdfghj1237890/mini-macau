...
- **`/api/dsat/batch` 由 Vite plugin 提供**：[`vite.config.ts`](../../vite.config.ts) 的 `dsatBatchDevPlugin` 是這個 batch endpoint 唯一的實作，負責把多路 DSAT 請求 fan-out 出去、合併成單一 JSON 陣列。這代表你在本機就能測 RT mode。
