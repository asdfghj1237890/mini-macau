...
**路口區段另存一檔（2026-09-25）**：`build-bus-road-profile.mjs` 把各路線的路口區段寫到 `public/data/bus-junctions.json`（`{version: 1, routes: {<路線 id>: {geometryKey, junctions}}}`），`bus-routes.json` 的 `roadProfile` 只留路段。路口只影響街道層級的細部交通，不影響班表與車輛的時刻表位置，所以前端在核心資料之後以低優先權載入，`geometryKey` 相符才原地掛回已載入的路線物件（`attachBusJunctions`，`src/engines/busJunctions.ts`）；`AsyncBusFrame` 把新掛上的路口轉給 worker，並像拖曳時間一樣重置一次交通佇列。`validate_output.py bus-junctions` 檢查每條有路段資料的路線都有一筆、`geometryKey` 相符，而 `bus-routes.json` 裡若還有 `junctions` 會報錯。`inspect.mjs`、replay 測試與 `capture-bus-replay-routes.mjs` 讀資料時會先合併這個檔案。
