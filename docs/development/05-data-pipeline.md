...
古地圖支援本機配準影像與外部 WMTS 圖磚。**1996 澳門地區全圖**由 `scripts/old-maps-remote.mjs` 定義，建置只保留服務的 URL、bounds 與 z0–17／256 px 資料，不下載或重新配準圖磚。MapLibre 與 Leaflet 備用模式均直接讀取中研院服務；沒有本機影像尺寸、GCP 或 RMS。圖源聲明未經許可不得商用，影像不屬於本專案的 MIT 授權。來源失效或離線時此圖層不可用。
...
**完整圖版顯示（2026-09-20，取代上文裁切流程）**：八張本機掃描改為保留完整來源圖版，包含紙邊、標題、圖例及插圖；1792 的兩張完整掃描旋轉後上下拼接，維持原有 68 px 書溝間隙。`registrationOrigin` 只保留先前配準座標的原點，不再裁圖。控制點、樣條／仿射及海岸貼合都不變，取樣時才將配準像素轉回完整來源像素；單圖縮圖與原尺寸瓦片共用此轉換。輸出範圍由原圖四邊逐點反解既有配準求得，保留舊輸出像素格網並檢查邊界未截斷圖面；只移除配準後矩形外的透明空間。1927 插圖與全景畫不再遮罩。1963 保留 2× 放大與輕度銳化。1996 為遠端 WMTS，本站沒有額外裁剪，沿用供應方提供的完整服務範圍。
