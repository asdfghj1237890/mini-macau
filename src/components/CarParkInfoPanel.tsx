...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-3">
      ...
      <span className="text-[10px] text-white/80 text-right mm-han min-w-0 whitespace-pre-line">
        {value}
      </span>
    </div>
  )
...
function FeeRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-3">
      <span className="text-[10px] text-white/45 shrink-0 mm-han">{label}</span>
      <span className="text-[10px] text-white/80 text-right mm-han min-w-0 whitespace-pre-line">
        {value}
      </span>
    </div>
...
function VacancyCell({ label, value }: { label: string; value: number | null }) {
  ...
  return (
    <div className="flex items-baseline justify-between gap-2">
      <span className="text-[10px] text-white/45 mm-han truncate">{label}</span>
      <span className="mm-mono mm-tabular text-[11px] text-blue-200 shrink-0">{value}</span>
    </div>
  )
}
  ...
  return (
    <div className="absolute top-16 left-4 z-20 w-[340px]
      ...
      <div className="bg-[#0b0b0c]/95 backdrop-blur-md border border-white/10 rounded-sm
                      shadow-2xl shadow-black/60 overflow-hidden mm-fade max-h-[70vh] overflow-y-auto">
        ...
        <div className="flex items-stretch border-b border-white/10">
          <div className="px-3 py-2 flex items-center gap-2 border-r border-white/10"
            ...
            <div>
              ...
              <div className="mm-mono text-[13px] font-bold text-white leading-tight">P</div>
            </div>
          </div>
          <div className="flex-1 px-3 py-2 flex flex-col justify-center min-w-0">
            <div className="text-[14px] font-bold text-white truncate mm-han" title={name}>
              {name}
            </div>
            {(zone || parish) && (
              <div className="text-[10px] text-white/45 truncate mm-han">
        ...
        {hasFees && (
          <div className="px-3 py-2 border-t border-white/8 space-y-1">
            ...
            {fees.remark && (
              <div className="pt-1 text-[9px] leading-[1.35] text-white/40 mm-han whitespace-pre-line">
                {fees.remark}
              </div>
            )}
          </div>
        ...
        <div className="px-3 py-2 border-t border-white/8 bg-white/[0.02]">
          ...
          {!polling ? (
            <div className="pt-1 text-[10px] text-white/40 mm-han">
              {t.carParkVacancyOnlyAtRealtime}
            </div>
          ) : paused ? (
            <div className="pt-1 text-[10px] text-amber-200/70 mm-han">
              {t.carParkVacancyPaused}
            </div>
          ) : !vacancy ? (
            <div className="pt-1 mm-mono text-[11px] text-white/35">…</div>
          ) : (
