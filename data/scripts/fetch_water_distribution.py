...
end nearer a treated-water source to the end further away, so a dashed line
animated along the array reads as water flowing outward from the plants. The
distance field is a multi-source Dijkstra over the road graph, seeded at the
treated-water facilities in water-facilities.json (plants, elevated tanks and
treated-water pumping stations — NOT the raw-water side, which feeds the
plants rather than the city). `dist` / `distEnd` carry the metres at each end.
Roads in a component no source can reach keep their original order and get
`dist: null`.
...
# Treated-water sources for the flow field. The raw-water side (reservoirs and
# `raw_pumping`) is deliberately excluded: raw water flows INTO the plants, so
# seeding it would make half the city appear to drain towards 大水塘. The
# operator filter drops 黑沙水庫 (DSAMA) with the rest of the raw side.
...
# mis-placed or genuinely off-street (a tank on a hilltop); print it either way.
SNAP_WARN_M = 150.0

# Degenerate-run guards, mirrored by validate_output.py's v_water_distribution.
MIN_ROADS = 2000
...
def load_sources() -> list[dict]:
    """Treated-water facilities, from the sibling dataset."""
...
def snap(nodes_xy, facility) -> tuple[tuple[float, float], float]:
    """Nearest graph node to a facility marker, and how far it had to reach."""
    fx, fy = metres_xy(facility["coordinates"][0], facility["coordinates"][1], LAT0)
    best = None
    best_d = None
...
def run() -> int:
    ...
    facilities = load_sources()
    print(f"\nFlow sources: {len(facilities)} treated-water facilities "
          f"(operator {FLOW_SOURCE_OPERATOR}, types {sorted(FLOW_SOURCE_TYPES)})")

    starts = []
    source_ids = []
    worst_snap = 0.0
    for f in sorted(facilities, key=lambda x: x["no"]):
        node, d = snap(nodes_xy, f)
        starts.append(node)
        ...
        print(f"  {f['id']:<20} {f['type']:<8} snap {d:6.1f} m{flag}")
    ...
    if not source_ids:
        problems.append("no treated-water sources found in water-facilities.json")
    if problems:
        for p in problems:
