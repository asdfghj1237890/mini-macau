...
That machinery — Overpass access, OSM outline -> footprint, and the re-cut
against the basemap's own building parts — lives in `osm_footprints.py` and is
shared with `fetch_water_facilities.py`; this file only decides which buildings
belong to which school.
...
# footprints come from the tiles rather than from OSM alone.
...
# The shared Macau bbox spills into Zhuhai / Hengqin; the bus-stop proximity
# test below trims that side.
BBOX = MACAU_BBOX
...
def run() -> int:
    ...
    tiles = tiles_covering(
        [rec["_poly"] for recs in assigned.values() for rec in recs] + [poly for _, poly in fallback]
    )
    ...
    for key, poly in fallback:
        ...
        if inside:
            for part in inside:
                claim(key, part_record(part, lat0, part["id"], None, "tile"))
