...
[![MapLibre GL](https://img.shields.io/badge/MapLibre_GL-6-396CB2?logo=maplibre&logoColor=white)](https://maplibre.org/)
...
- **3D Aircraft** — about 160 real MFM flights (auto-synced daily) with detailed airplane models, apron stands, and taxi paths
- **3D Ferries** — 6 HK/Shenzhen ↔ Macau sea routes (TurboJET + CotaiJet) with a twin-hull catamaran shape, raised bridge, and multi-deck cabin
...
<details>
<summary><strong>Transit & simulation</strong></summary>
...
- **3D Aircraft** — about 160 real MFM airport flights (auto-synced daily) with detailed airplane models (fuselage, swept wings, vertical tail in airline colors, engine nacelles, window rows, cockpit windshield); aircraft park at 12 apron stands before departure and taxi along waypoint paths before takeoff
...
- **3D Ferries** — 6 sea routes (Hong Kong Outer Harbour / Taipa / Sheung Wan, HKIA, Shenzhen Airport, Shekou) served by TurboJET and CotaiJet, rendered as a twin-hull catamaran model (flared hulls, raised bridge with wraparound windscreen, cabin, windows, roof equipment) following waypoint routes between berth and open water, with berth-facing headings while docked
...
<details>
<summary><strong>City layers</strong></summary>
...
- **Public car parks** — DSAT's 92 public car parks as map markers with entrances, height limits and fees, plus live vacancy shown only while the clock is at the present; toggleable
...
- **Responsive mobile UI** — Hamburger menu for map controls, a chip stack for LRT, Bus and a combined Air & Sea chip plus one CITY chip that opens a shared sheet listing all twelve city layers, with a detail view per layer, optimized touch layout with safe-area support, and Add to Home Screen (a web app manifest plus an install card that names the Share / menu route for iOS and Android browsers)
- **Lazy loading** — Code-split panels (VehicleInfoPanel, StationInfoPanel, FlightInfoPanel, FerryInfoPanel, RoadWorkInfoPanel, SchoolInfoPanel, PublicHousingInfoPanel, ParishInfoPanel, ToiletInfoPanel, ReligionInfoPanel, CarParkInfoPanel, WasteSiteInfoPanel, WasteIncineratorInfoPanel, WasteEcoStationInfoPanel, WasteFacilityInfoPanel, WaterFacilityInfoPanel, WaterInletInfoPanel, PowerFacilityInfoPanel, PowerInletInfoPanel, GrandPrixCircuitInfoPanel, GrandPrixCornerInfoPanel), plus the map component itself (MapView), for fast initial load
...
<details>
<summary><strong>Diagnosing a device</strong></summary>
...
On context loss or shader failure the app rebuilds the map once, retaining its camera. Repeated failure switches to a 2D compatibility map with routes, moving vehicle points, and every city layer — toilets, car parks, waste & recycling sites, water and power facilities with their schematic networks, schools, public housing, religion sites, road works, the parish tint, the Grand Prix circuit and car, and historical-map plates — plus the existing time/layer controls. It does not require WebGL; 3D buildings and animated network flows are unavailable. Debugging observes the map's existing context without creating probe contexts, and reports null shader query results separately from false compile status. See [recovery behavior and investigation](docs/development/11-webgl-recovery.md).
...
# Run the extraction scripts (there is no single entry point — see below)
uv run python scripts/extract_lrt_osm.py
uv run python scripts/fetch_bus_data.py
uv run python scripts/fetch_dsat_stops.py
uv run python scripts/fetch_bridge_geometry.py
uv run python scripts/extract_bus_data.py
uv run python scripts/patch_bus_bridges.py
uv run python scripts/patch_service_hours.py
uv run python scripts/patch_service_hours_by_day.py
```

Then, from the repo root, run the Node post-processing chain:
...
2. Fetch [motransportinfo.com](https://www.motransportinfo.com) route/stop reference data, DSAT's authoritative per-direction stop lists and the bridge approach geometry into `data/bus_reference/`
3. Build bus routes and stops from that reference data, snapping each route to the roads via OSRM (`osrm_route.py` is the helper module `extract_bus_data.py` calls)
4. Splice the custom bridge geometry into the cross-harbour routes
5. Recompute bus service hours and frequency from the published (weekday / Saturday / Sunday) schedules
6. `npm run data:routes` patches the Amaral terminal layout, corrects routes against MO Transport's published GPX traces, and rebuilds the road-speed profile

The reference fetchers write to `data/bus_reference/`; everything the app loads is written straight to where it is consumed, `public/data/`, and served as-is. There is no intermediate `data/output/` copy to sync.
...
This is also automated via GitHub Actions (`.github/workflows/update-flights.yml`), which runs daily at 07:07 Macau time (UTC+8), plus up to 50 minutes of random delay, and commits updated flight data if changed.

A second workflow (`.github/workflows/update-flights-timetable.yml`) fetches a 7-day timetable window starting tomorrow into a separate `public/data/flights-timetable.json`, so browsing a future date with the DateTimePicker shows real scheduled flights beyond what the daily sync covers. The frontend merges both files in `buildFlightIndex` (`src/hooks/useTransitData.ts`); today's flights always come from the realtime `flights.json`.
...
- **1990 / 1991 survey sheets** — The Direcção dos Serviços de Cartografia e Cadastro’s *Macau* 1:5,000 (1990) and *Território de Macau* 1:20,000 (1991), from the Library of Congress Geography and Map Division ([92684644](https://www.loc.gov/item/92684644/), [92684636](https://www.loc.gov/item/92684636/)) via Wikimedia Commons, listed with no known copyright restrictions. Both are registered on their printed Macau grid: grid crosses read on the scan, converted from Macao Grid (EPSG:8433) to WGS 84 with EPSG:8438, one affine fit per sheet (`scripts/old-maps-dscc.mjs`).
...
| **Flights** | Static daily sync | [AviationStack API](https://aviationstack.com/) | Daily around 07:07–07:57 Macau time — `update-flights.yml`; plus a 7-day-ahead timetable — `update-flights-timetable.yml` | `fetchedAtUtc` embedded in `flights.json` / `flights-timetable.json` |
...
│   │   ├── FontSizeControl.tsx   # Text-size slider control
│   │   ├── LineLegend.tsx        # Layer legend — desktop TRANSIT/CITY pages + mobile chips
│   │   ├── CityLayerList.tsx     # Desktop CITY layer rows + per-layer detail
│   │   ├── CityLayerLoadState.tsx # Loading/error indicator for a city dataset
│   │   ├── MobileLayerSheet.tsx  # Shared phone sheet — LRT/Bus/Air&Sea/CITY tabs + per-layer detail
│   │   ├── MobileLayerIcon.tsx   # SVG icon set for the phone chip stack
│   │   ├── MobileBusRegister.tsx # Phone bus route-group list
│   │   ├── MobileLrtConsole.tsx  # Phone LRT line/station console
│   │   ├── MobileServiceTickets.tsx # Phone bus service-status tickets
│   │   ├── OldMapControls.tsx    # Historical-map row, opacity slider, notes/sources
│   │   ├── OldMapSwitcher.tsx    # Phone on-map ‹ year › stepper for historical maps
...
│   │   ├── WaterFacilityInfoPanel.tsx # Water facility + inlet detail panels (2 exports)
│   │   ├── PowerFacilityInfoPanel.tsx # Power facility + inlet detail panels (2 exports)
│   │   ├── GrandPrixInfoPanel.tsx # Circuit + corner detail panels (2 exports)
│   │   ├── StatsChart.tsx        # Shared monthly bar chart (incinerator/hazardous/landfill/WWTP panels)
│   │   ├── MapSplash.tsx         # Splash shown while the MapView chunk loads
│   │   ├── PwaInstallPrompt.tsx  # Add-to-Home-Screen install card
│   │   ├── RasterMapFallback.tsx # 2D Leaflet/Canvas2D fallback map (WebGL recovery)
│   │   └── TransitIcons.tsx      # Shared train/bus/close icon set
│   ├── engines/
│   │   ├── simulationEngine.ts   # Timetable-driven vehicle + flight position computation
│   │   ├── vehicleFrame.ts       # Per-frame position computation shared by MapView + 2D fallback
│   │   ├── asyncBusFrame.ts      # Main-thread bridge to the bus-traffic worker
│   │   ├── busWorkerRuntime.ts   # Bus-traffic worker runtime (see Performance Notes)
│   │   ├── busTraffic.ts         # Bus following, junction reservations, swept-body checks
│   │   ├── busTraffic.worker.ts  # Module-worker entry point wrapping busTraffic
│   │   ├── busTrafficScope.ts    # Viewport-scoped detailed traffic vs. schedule-only distant buses
│   │   ├── busJunctions.ts       # Junction reservation zones + chaining
│   │   ├── busWaitGroups.ts      # Strongly-connected waiting-group recovery
│   │   ├── busLaneGeometry.ts    # Per-lane offset geometry cache
│   │   ├── busRoadProfile.ts     # Road classification lookup (speed, lanes, dividers)
│   │   ├── busPlayback.ts        # Interpolates the fleet on one shared playhead
│   │   ├── busPlaybackRate.ts    # Drops the selected clock speed when the worker falls behind
│   │   ├── busMotionTrace.ts     # BusTraceRecorder: one worker request's checked bus trajectories, handed to playback
│   │   ├── ferryBerths.ts        # Berth moor points + bow-target bearings for the two terminals
│   │   ├── ferryRoutes.ts        # Outbound waypoint paths from the Outer Harbour Terminal
│   │   ├── lrtMotion.ts          # LRT acceleration/deceleration + speed cap
│   │   └── lrtTimetable.ts       # LRT timetable-entry helpers
│   ├── data/
│   │   └── hourDensity.ts
│   ├── hooks/
│   │   ├── useSimulationClock.ts # RAF-based clock with speed/pause
│   │   ├── useTransitData.ts     # JSON data loader
│   │   ├── useLrtState.ts        # LRT state-window fetch/prefetch (/api/lrt/state)
│   │   ├── useCarParkVacancy.ts  # Live car-park vacancy polling (1x + tab visible only)
│   │   ├── useServiceStatus.ts   # Daily bus service-status snapshot loader
│   │   ├── useWaterDistribution.ts # Lazy-loaded water distribution network
│   │   ├── usePowerDistribution.ts # Lazy-loaded power distribution network
│   │   └── usePwaInstall.ts      # Add-to-Home-Screen install-state hook
│   ├── layers/
...
│   ├── mapRecovery.ts            # WebGL context-loss/rebuild coalescing (2D fallback trigger)
│   ├── webglDiagnostics.ts       # ?debug=1 WebGL capability probes (observes the map's own context)
│   ├── routeGroups.ts            # Bus route grouping logic
│   ├── lrtTracks.ts              # LRT track geometry + offset helpers
│   ├── lrtViaduct.ts             # LRT elevated-viaduct outline geometry
│   ├── lrtState.ts               # LRT state-window schema + window-boundary helpers
│   ├── lrtStateStore.ts          # Fetches/prefetches LRT state windows from /api/lrt/state
│   ├── cityData.ts               # CityLayer / CityDataset keys + per-layer load status
│   ├── cityCatalog.ts            # Build-time city-layer dataset counts (virtual:city-catalog)
│   ├── cityDataFetch.ts          # City-layer JSON fetch + zod parse helpers
...
│   ├── oldMaps.ts                # Historical-maps overlay helpers (ids, names, single selection, opacity)
│   ├── oldMapGroups.ts           # Historical-map selector row grouping (e.g. the 1912 atlas trio)
│   ├── oldMapLabels.ts           # Basemap label paint/layout overrides while a historical map is shown
...
│   ├── fontSize.ts               # Persisted UI text-size store
│   ├── theme.ts                  # dark / light theme store (data-theme on <html>)
│   ├── timeControls.ts           # Clock shortcut guard for native controls
│   ├── pwaInstall.ts             # Add-to-Home-Screen browser detection + install-event store
│   ├── thankYou.ts               # /thank-you landing page copy + shared engagement analytics
...
├── plugins/
│   ├── city-catalog.ts       # Build-time CityLayer dataset counts (virtual:city-catalog)
│   ├── lrt-dev-api.ts        # Dev-only stand-in for the Function above
│   └── seo-content/          # Build-time SEO content injection into index.html (own tests)
├── .github/workflows/
│   ├── ci.yml                       # Lint + test + build + data validation (every PR/push)
...
│   ├── update-flights-timetable.yml # Daily 7-day-ahead flight timetable update
...
│   ├── update-power-facilities.yml # Twice-yearly power-facility update
│   └── update-water-facilities.yml # Twice-yearly water-facility update
└── index.html
```
