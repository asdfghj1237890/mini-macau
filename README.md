...
Visualizes the **Macau Light Rapid Transit (LRT)**, **bus network**, **HK–Macau ferry routes**, and **MFM airport flights** on an interactive 3D map. Vehicles move along actual geometry in a **timetable-driven simulation**. A **CITY** layer set adds Macau's open data on top: DSAT road-works notices, every school's buildings coloured by level and shaded by founding era, the Housing Bureau's social and economic housing estates, the seven civil parishes as a boundary tint, IAM public toilets, the Earth God's (土地公) temples and street shrines, DSAT public car parks with live vacancy, and IAM/DSPA refuse rooms, compacting bins and recycling points.
...
- **Religion — Tou Tei (土地公)** — the Earth God's temples (土地廟 / 福德祠) and street shrines (土地神壇) as map markers, merged from three sources: exact points from OpenStreetMap, the six sites the Cultural Affairs Bureau (IC) has classified as immovable heritage (with the IC's own trilingual name and description), and the sites photographed for Macau Memory's 社區守護神 exhibition, whose positions are street-level geocodes and are drawn dimmer and flagged approximate. Click a marker for its name (the inscription on the tablet, in English or Portuguese only where the IC publishes one), type, address, heritage code and the sources it came from. The IC counts about 10 temples and 160+ public shrines; the layer maps the ones with a known location and says so; toggleable
...
| City data | [data.gov.mo](https://data.gov.mo/) — DSAT road works, DSAT car parks + live vacancy (daily syncs); IAM toilets, IAM/DSPA waste & recycling points (monthly syncs); DSEDJ school list, IH's public-housing lists, Macao Water's facility list and CEM's substation list, all + OSM footprints (manual); DSEC 2021 census + OSM boundaries for the parishes (manual); OpenStreetMap + the IC cultural-heritage API + Macau Memory's exhibition map for the Tou Tei temples and shrines (manual) |
...
- **Religion — Tou Tei** — [OpenStreetMap](https://www.openstreetmap.org/copyright) places of worship named for 土地 / 福德 / 社稷 (positions), the [IC 文化遺產資料 API via data.gov.mo](https://data.gov.mo/Detail?id=7e1eca8e-6ffe-4f74-8c81-25c25beb45b2) (the six classified sites: official GPS and trilingual descriptions), and Macau Memory's [社區守護神 exhibition](https://www.macaumemory.mo/exhibitions/showexhibition!toSep?id=8c35d71325374eeda344f11a351a27d7) map (names, street addresses and record ids only — its positions are Google geocodes of a street name, flagged approximate; no photos or text are copied) (manual refresh)
...
│   ├── religion.ts               # Tou Tei overlay helpers (kind, colours, marker features)
...
│   │   ├── religion.json         # Tou Tei temples and shrines (OSM + IC heritage API + Macau Memory)
...
│   │   ├── fetch_religion.py     # Tou Tei temples and shrines (manual)
