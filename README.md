...
- **Waste & recycling** — IAM's refuse rooms, compacting bins and refuse stations plus DSPA's smart recycling machines, three-colour recycling points, e-waste points and lamp/battery points (≈1,136 sites), DSPA's 10 Eco Fun drop-off stations, and a treatment-facilities row covering the 澳門垃圾焚化中心 incineration plant (coloured 3D buildings — the same OSM footprints the electricity layer uses — plus its monthly throughput stats), the hazardous-waste station and two landfills (filled outlines) — ≈1,150 marks across nine toggleable key rows. A focus mode like water and power (the three are mutually exclusive): switching it on hides every other layer — LRT, buses, air, sea, road works, schools, toilets, car parks — along with the clock and time controls, and restores them exactly as they were when it's switched off The five recycling rows (smart machines, three-colour, e-waste, lamp/battery, Eco Fun stations) start hidden so the collection points are not buried; switch them on from the key.
