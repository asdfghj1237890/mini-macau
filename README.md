...
Both the 3D map and the 2D fallback calculate bus following, junction reservations and swept-body checks in a module worker. Accelerated playback uses aligned batches, with route traces transferred back for interpolation between replies. Visible models use detailed lane samples and an additional presentation collision check. There is at most one request in flight: clock updates coalesce instead of building a queue of obsolete frames. Catch-up is bounded to 16 simulated seconds; longer stalls resynchronize with a checked placement instead of accumulating replay work. Seeks and layer changes reject stale replies; pausing flushes the final checked position. If workers are unavailable, the traffic engine runs synchronously.
